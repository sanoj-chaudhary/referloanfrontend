  ...
  try {
    ...
    if (query_cat) {
      for (let i in query_cat) {
        ...
        {
          query_product = await db.query("SELECT id,name , slug FROM `view_product` WHERE `cat_id` = '" + category_id + "' AND `status` = '1' and `is_menu` = '1' and `slug` != '' ORDER BY `order` ");
          ...
          if (query_product) {
            for (let j in query_product) {
              ...
              query_bank_product = await db.query("SELECT id,name, slug FROM `view_bank_product` WHERE `status`='1' and `product_id` = '" + product_id + "' ORDER BY `order_by`   ");
